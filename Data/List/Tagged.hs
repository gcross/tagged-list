...
-- @@language Haskell
-- @<< Language extensions >>
-- @+node:gcross.20100917213812.1279:<< Language extensions >>
...
-- @+node:gcross.20100917213812.1280:<< Import needed modules >>
...
-- @+others
-- @+node:gcross.20100917213812.1281:Types
-- @+node:gcross.20100917213812.1282:TaggedList
...
-- @-node:gcross.20100917213812.1287:Functor TaggedList
-- @+node:gcross.20100917213812.1288:Foldable TaggedList
...
-- @-node:gcross.20100917213812.1288:Foldable TaggedList
-- @+node:gcross.20100917213812.1289:Eq
...
-- @-node:gcross.20100917213812.1289:Eq
-- @-node:gcross.20100917213812.1284:Instances
-- @+node:gcross.20100917213812.1290:Functions
-- @+node:gcross.20100917213812.1291:length
length :: NaturalNumber n => TaggedList n a → N n
length _ = asN
-- @-node:gcross.20100917213812.1291:length
-- @+node:gcross.20100917213812.1292:head
head :: TaggedList (SuccessorTo n) a → a
head (x :. _) = x
-- @-node:gcross.20100917213812.1292:head
-- @+node:gcross.20100917213812.1293:tail
tail :: TaggedList (SuccessorTo n) a → TaggedList n a
tail (_ :. xs) = xs
-- @-node:gcross.20100917213812.1293:tail
-- @+node:gcross.20100917213812.1294:append
append :: TaggedList m a → TaggedList n a → TaggedList (Plus m n) a
append E = id
append (x :. xs) = (x :.) . append xs
-- @-node:gcross.20100917213812.1294:append
-- @+node:gcross.20100917213812.1295:join
...
-- @-node:gcross.20100917213812.1295:join
-- @+node:gcross.20100917213812.1296:replace
...
-- @-node:gcross.20100917213812.1296:replace
-- @+node:gcross.20100917213812.1297:unwrapRights
...
unwrapRights (Right x :. rest) =  
    case unwrapRights rest of
        ...
        Right xs → Right (x:.xs)
-- @-node:gcross.20100917213812.1297:unwrapRights
-- @+node:gcross.20100917213812.1298:map
map :: (a → b) → TaggedList n a → TaggedList n b
map f E = E
map f (x :. xs) = f x :. map f xs
-- @-node:gcross.20100917213812.1298:map
-- @+node:gcross.20100917213812.1299:toList
toList :: TaggedList n a → [a]
toList E = []
toList (x :. xs) = x : toList xs
-- @-node:gcross.20100917213812.1299:toList
-- @+node:gcross.20100917213812.1300:fromListAsUntagged
...
-- @-node:gcross.20100917213812.1300:fromListAsUntagged
-- @+node:gcross.20100917213812.1301:fromList
...
-- @-node:gcross.20100917213812.1301:fromList
-- @+node:gcross.20100917213812.1302:eqLists
...
-- @-node:gcross.20100917213812.1302:eqLists
-- @+node:gcross.20100917213812.1303:mapM
...
mapM f (x :. xs) = do
    ...
    return (y :. ys)
-- @-node:gcross.20100917213812.1303:mapM
-- @+node:gcross.20100917213812.1304:mapM_
mapM_ :: Monad m => (a → m b) → TaggedList n a → m ()
mapM_ f E = return ()
mapM_ f (x :. xs) = f x >> mapM_ f xs
-- @-node:gcross.20100917213812.1304:mapM_
-- @+node:gcross.20100917213812.1305:zipf
